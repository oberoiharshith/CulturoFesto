...
class FestivalAdmin extends React.Component {
  ...
  deleteRow(festivalId, e) {
    axios
      .delete(`http://3.20.221.44:8082/festival/delete/${festivalId}`)
      .then((response) => {
        if (response.data != null) {
          alert(response.data);
          this.setState({
            festivals: this.state.festival.filter(
              (festival) => festival.festivalId !== festivalId
            ),
          });
        }
      });
  ...
  render() {
    ...
    return (
      <Card className="border border-dark bg-dark text-white text-center">
        <Card.Header>
          {" "}
          <FontAwesomeIcon icon={faList} /> Editable Festival
        </Card.Header>
        <Card.Body>
                    ...
                    <td>
                      <ButtonGroup>
                        ...
                        <Button
                          ...
                          onClick={(e) =>
                            this.deleteRow(festival.festivalId, e)
                          }
                        >
